...
def generate_model(model, x_test, name='weights.h', format='hwc', quantize_method='max_min'):
    ...
    with open(name,'a') as fp:
        ...
        for d in model.input.shape[1:]:
            sz = sz*d
        fp.write('static int8_t microinfer_input_data[%d];\n'%(sz))
        sz = 1
        for d in model.output.shape[1:]:
            sz = sz*d
        fp.write('static int8_t microinfer_output_data[%d];\n'%(sz))
        fp.write('static microinfer_model_t* microinfer_model_create(void)\n{\n')
        fp.write('\tstatic microinfer_model_t model;\n')
        if(ID>32):
            fp.write('\tmicroinfer_layer_t ** layer = malloc(sizeof(microinfer_layer_t *)*%d);\n'%(ID+1))
            fp.write('\tif(NULL == layer) return NULL;\n')
        else:
            fp.write('\tmicroinfer_layer_t* layer[%d];\n'%(ID+1))
        fp.write('\n\tnew_model(&model);\n\n')
        for layer in L:
            if(is_skipable_layer(layer)):
                continue
            ...
            if('input' in layer.name):
                ...
                if (len(inshape) == 1):  # 1-D input
                    fp.write('\tlayer[%d] = Input(shape(%d,1,1), microinfer_input_data);\n' % (id, inshape[0]))
                elif (len(inshape) == 2):  # 1-D input
                    fp.write('\tlayer[%d] = Input(shape(1,%d,%d), microinfer_input_data);\n' % (id, inshape[0], inshape[1]))
                else:
                    fp.write('\tlayer[%d] = Input(shape%s, microinfer_input_data);\n' % (id, inshape))
        ...
        if('softmax' in layer.name
           or ('activation' in layer.name and layer.get_config()['activation'] == 'softmax')):
            fp.write('\tlayer[%s] = model.hook(Output(shape(%s,1,1), microinfer_output_data), layer[%s]);\n'%(id+1, layer.output.shape[1], id))
        elif len(layer.output.shape) == 4:
            fp.write('\tlayer[%s] = model.hook(Output(shape%s,microinfer_output_data), layer[%s]);\n'%(id+1, layer.output.shape[1:], id))
        elif len(layer.output.shape) == 3:
            fp.write('\tlayer[%s] = model.hook(Output(shape(1,%s,%s), microinfer_output_data), layer[%s]);\n'%(id+1, layer.output.shape[1], layer.output.shape[2], id))
